...
def reduced_gravity(rho1, rho2, g: float = G0, ref: str = "upper"):
    ...
    if ref not in den:
        raise ValueError("ref must be 'upper' (ch04, rho1), 'lower' (ch07 (7.117), rho2) or 'mean'")
    return _S(float(g) * (_F(rho2) - _F(rho1)) / den[ref])

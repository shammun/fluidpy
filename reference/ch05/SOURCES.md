...
`tests/book_values_ch05.json`). Label use in `tests/test_ch05.py`: every entry here is a published closed-form formula that our code reproduces
identically, so all are V1 "form cross-checks against a published formula" (not V5; review follow-up).

| value / form | what it is | source | URL | how obtained | verified | label |
|---|---|---|---|---|---|---|
| `kelvin_ring_speed` | U = Γ/(4πR)[ln(8R/a) − 1/4], thin core of uniform vorticity (Kelvin) | Wikipedia, "Vortex ring" | https://en.wikipedia.org/wiki/Vortex_ring | page read; formula and "thin-core, uniform" qualifier quoted | 2026-09-23 | V1 form (published formula) |
| `point_vortex_pairs` | corotating pair Γ₁ = Γ₂ = 1 at ±l rotates at Ω₀ = 1/(4πl²); counter-rotating pair (1 at +l, −1 at −l) translates at V₀ = −i/(4πl) | C. García & S. V. Haziot, "Global bifurcation for corotating and counter-rotating vortex pairs", Commun. Math. Phys. (2023), doi:10.1007/s00220-023-04741-6 | https://arxiv.org/html/2204.11327 (§2.1) | HTML full text read; formulas quoted | 2026-09-23 | V1 form (published formula) |

# Chapter 2 verification — Cartesian Tensors                     2026-09-16, commit ebc80e3 (+ this phase's uncommitted tests/reference/report)
...
Runs: `pytest tests/test_ch02.py -q -p no:cacheprovider` → **91 passed, 0 failed, 1 xfailed (strict, Open item O1) in 42 s**
(with the private book file; without it 4 V6 tests skip). Full suite `pytest -q` → **206 passed, 1 xfailed in 62 s**
(ch01's 107 tests still pass with `traction_components` re-exported from `core.tensors`). All 14 runnable
...
| C14 Gauss' theorem (2.30); N64, N65; D25 | CORE | `divergence_theorem_box`, `gauss_gradient_box` (scalar Q), `flux_through_box_faces`, `volume_integral_box`, `divergence_theorem_sphere`, `flux_through_sphere`, `divergence_theorem_rect2d`, `flux_through_faces`, `divergence_theorem_tiled`, quadrature nodes | V1 (x, y, z) → 3 = 3, (x², 0, 0) → 1 = 1, face keys/sum, scalar xyz gradient form ¼, 2-D 2 = 2 and 1 = 1, Simpson/Gauss node exactness; V3 midpoint 2.002, Simpson 4.00, 2-D 2.0, FD-divergence fallback 1e-10; V5 F = (2x, y², z²) through the unit sphere = 8π/3 (both sides rel 4e-16 / 8e-16; FD fallback 1e-9; shifted centre 8π/3(1 + c_y + c_z)); V4 solenoidal b×x: zero net outflux through boxes, spheres, rectangles and tiles (< 1e-13); point source m through enclosing boxes = m, non-enclosing 0; V2 D25 FTC face pair for a symbolic polynomial Q (all three i), shared-face cancellation symbolically, numeric tiling interior −5.6e-17 | see tables | analytic, symbolic, converged, conserved, benchmark | Open item O1 (vector Q in `gauss_gradient_box`) |
...
## Open items
- **O1 (implementer, minor):** `core.integral_theorems.gauss_gradient_box` promises in its docstring "(3, 3) for vector Q
  ([i, j] = ∂Q_j/∂x_i)", but a vector Q raises `ValueError: input operand has more dimensions than allowed by the axis
  remapping` in `volume_integral_box` — `_eval` only broadcasts outputs with at most one leading component axis, and
  `fd_partials` / an exact `dQ_fn` return a rank-2 field `(3, 3, …)`. Proven by
  `test_gauss_theorem_V1_vector_Q_gradient_form_promised_by_docstring` (strict xfail; it will start failing — i.e. must be
  un-xfailed — once fixed). Design Part C 5.1 only requires the scalar-Q case, which passes; nothing in the notebook or
  explainer storyboards calls the vector case. Fix: let `_eval` broadcast any leading axes (`out.ndim > B[0].ndim` →
  reshape/broadcast the trailing axes), or narrow the docstring.
...
## Verdict: PASS
(with Open items O1–O4 to be routed: O1 minor code fix or docstring narrowing; O2 docstring wording; O3/O4 design
text/check-cell corrections before the notebook is built.)

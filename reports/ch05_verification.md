...
V4 8 · V5 2 · V6 4** (= 133) in loops 1–2; after the review follow-up **V1 90 · V2 23 · V3 10 · V4 8 · V6 4**
(= 135; see the last section).
...
| Kelvin ring speed with the hollow-core constant for 'uniform' | 1 — test_ring_self_velocity_V5_kelvin_thin_ring (renamed `…_V1_kelvin_thin_ring_form` in the review follow-up) |
...
| N40–N41 pairs | `vortex_pair`, `point_vortex_evolve` | V1 (incl. form cross-check), V6 | analytic, book-value |
| N42–N43 rings | `ring_dynamics`, `ring_ring_velocity`, `ring_self_velocity` | V4, V3, V1 (form cross-check; validity stop) | conserved, converged, analytic |
...
Review follow-up: none of these is used as V5 any more. Kelvin's ring speed and the García–Haziot pair rates are
published closed-form formulas that our code reproduces identically, so they are **V1 form cross-checks against a
published formula**, like the Wikipedia forms (ch04 convention). Chapter 5 has no V5 evidence; every CORE row still has
≥ 2 levels with V1/V2/V3 (C08: V1, V2, V3; C12: V1, V4).
| value | ours | source | verified |
|---|---|---|---|
| Kelvin thin-ring speed U = Γ/4πR[ln(8R/a) − ¼] (uniform core) | identical at three (R, a, Γ) (1e-14); 0.328816 m/s | Wikipedia "Vortex ring", https://en.wikipedia.org/wiki/Vortex_ring | 2026-09-23 (V1 form) |
| corotating pair Ω₀ = 1/(4πl²); counter-rotating V₀ = −i/(4πl) (vortices at ±l) | `vortex_pair` identical (1e-14); integrated angle and drift 1e-9 | García & Haziot, Commun. Math. Phys. (2023), doi:10.1007/s00220-023-04741-6, arXiv:2204.11327 §2.1, https://arxiv.org/html/2204.11327 | 2026-09-23 (V1 form) |
...
Reused: WGS-84 Ω (reference/ch04). All six are V1 "form cross-checks (not V5)", as in ch04.
...
wrong variants and the 4 re-planted F1/F2 variants are caught (and, after the review, the NaN centre image). F1 and F2 are fixed and re-checked independently
...
## Review follow-up (reports/ch05_review.md) — 2026-09-23
The review had no must-fix items; the implementer fixed the should-fix items (`ring_dynamics` validity stop,
`circle_image_system` for a vortex at the centre, docstring labels). The verifier edited only `tests/test_ch05.py`,
this report and `reference/ch05/` labels; no tolerance was loosened.

| change | test | numbers |
|---|---|---|
| Label honesty: Kelvin's ring speed and the García–Haziot pair rates are published *formulas* reproduced identically → V1 form cross-checks | `test_ring_self_velocity_V1_kelvin_thin_ring_form`, `test_point_vortex_pairs_V1_garcia_haziot_rates_form` (renamed from `_V5_…`) | unchanged (1e-14; 1e-9) |
| Circle image of a vortex at the centre (image at infinity, dropped) | `test_circle_images_V1_vortex_at_the_centre_has_no_finite_image` | `point_vortex_rhs([[0.3, 0.6], [0, 0.2]], [1, −1], "circle", a=1, center=(0.3, 0))` = [[−0.213023, −0.208266], [0.319534, 0.312400]], finite, equal to our hand sum (only vortex 2's inverse-point image, +1 at c + d/∣d∣²) to 1e-13; 3 vortices in the system; the circle stays a streamline (1e-13) |
| `ring_dynamics` validity stop (`min_gap` = 3, `terminate`) | `test_ring_dynamics_V1_validity_stop_where_gap_is_3a` | R = 1, a = 0.1, Γ = 1, wall z = 3: `stop_time` 9.404847 s, reason "wall"; our independent integration (Kelvin's published speed + our own M = 512 polygon image ring + our own gap = 3a event) 9.404783 s (−6.9e-6 relative), at R = 1.3353 m, z = 2.7404 m; `valid` = t < stop_time; valid samples have gap ≥ 3a, the first invalid < 3a; `terminate=True` returns the 48 samples before the stop, identical to the free run (1e-8); min_gap = 2 stops later |
| The 60-s wall run's physical claims restricted to `valid` samples | `test_ring_dynamics_V4_leapfrog_and_ring_toward_a_wall` | R monotone up and approach speed monotone down on the 48 valid samples (to 9.4 s), z < wall on all 301; speed at the stop < 0.5 × initial (0.1 × held only over the 60-s invalid tail — asserting it there would have claimed physics the model no longer has); leap-frogging rings: no stop (cores ≥ 3.69a apart), `valid` all true |

Discrimination (pre-fix behaviour planted back into a scratch copy): the NaN centre image (`fin = r2 ≥ 0`, i.e. the
image at c + a²d/0) → 1 test fails (`test_circle_images_V1_vortex_at_the_centre_has_no_finite_image`).

**`terminate=True` as the default? — not recommended.** With `terminate=False` the arrays keep the shape of `t_eval`
(the notebook, the B1 explainer and `scripts/ch05_point_vortices.py` index them by time), and `stop_time`/`valid` already
say where the model ends; with `terminate=True` the arrays silently shrink. Recommended instead: the notebook and
explainers draw only `valid` samples (or mark `stop_time`) and quote the stop — the leap-frog scene never triggers it.

Runs: `tests/test_ch05.py` → **135 passed** (V1 90 · V2 23 · V3 10 · V4 8 · V6 4); full suite `pytest -q` → **613 passed in 1156 s** (478 earlier + 135 ch05; slow wall time because other agents' browser audits shared the machine).
O3 is now handled in code (validity stop) and reduced to the notebook/explainer presentation note above.
